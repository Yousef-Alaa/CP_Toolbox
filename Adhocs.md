...
auto getInRange = [&](vector<vector<int>> &vec, int x1, int y1, int x2, int y2) {
	...
	return vec[x2][y2] - vec[x2][y1 - 1] - vec[x1 - 1][y2] + vec[x1 - 1][y1 - 1];
};

```


# <span style="color: #1ABC9C;font-style: italic;">Difference Array</span>
...
# <span style="color: #1ABC9C;font-style: italic;">2D Difference Array</span>
...
for (auto &q : opreratins) {
	...
	if (r2 + 1 < rows && c2 + 1 < cols) diff_2d[r2 + 1][c2 + 1] += v; // bottom-right -> add

}

// Prefix Rows
for (int i = 0; i < rows; i++) {
	for (int j = 1; j < cols; j++) diff_2d[i][j] += diff_2d[i][j - 1];
}

// Prefix Cols
for (int j = 0; j < cols; j++) {
	for (int i = 1; i < rows; i++) diff_2d[i][j] += diff_2d[i - 1][j];
}

// Add to original Matrix

for (int i = 0; i < rows; i++) {**
	for (int j = 0; j < cols; j++) matrix[i][j] += diff_2d[i][j];
}
